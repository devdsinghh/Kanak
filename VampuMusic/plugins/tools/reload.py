...
from VampuMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from VampuMusic.utils.decorators import ActualAdminCB, AdminActual, language
from VampuMusic.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Vampu.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Vampu.stop_stream_force(chat_id)
        except:
            pass
